...
async def next_page(bot, query):
    ...
    if int(req) not in [query.from_user.id, 0]:
        return await query.answer("mind your own business 😤", show_alert=True)
    try:
        offset = int(offset)
    except:
        offset = 0
    ...
    if not search:
        await query.answer("You Are Using My Old Messages🥲,Try Asking Again new 🤠.", show_alert=True)
        return
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        buttons = [[
            ...
            InlineKeyboardButton('♥️ Source (my creation)', callback_data='source')
        ], [
            InlineKeyboardButton('🏠 Home', callback_data='start'),
